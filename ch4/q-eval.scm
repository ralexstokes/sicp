...
(define (first-loop)
  ...
  (add-rule-or-assertion! (query-syntax-process '(rule (lives-near ?person-1 ?person-2)
...
(define (qeval query frame-stream)
  (let ((qproc (get (type query) 'qeval)))
    (if qproc
        (tap (qproc (contents query) frame-stream))
...
(define (map-over-symbols proc exp)
  (cond ((pair? exp)
         (cons (map-over-symbols proc (car exp)) (map-over-symbols proc (cdr exp))))
        ((symbol? exp) (proc exp))
        (else exp)))
